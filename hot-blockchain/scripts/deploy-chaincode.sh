#!/bin/bash
#
# deploy-chaincode.sh - Deploy chaincode to HOT blockchain
# Hyperledger Fabric v2.5.14 - Active Investigation Chain
#
# This script:
# 1. Packages the chaincode
# 2. Installs on LabOrg peer
# 3. Approves for LabOrg
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  HOT Blockchain Chaincode Deployment${NC}"
...
# ============================================================================
# STEP 2: Install Chaincode on LabOrg Peer
# ============================================================================

print_section "STEP 2: Installing chaincode on LabOrg peer0"
...
if [[ $? -eq 0 ]]; then
    print_success "Chaincode installed on peer0.laborg.hot.coc.com"
else
...
# ============================================================================
# STEP 3: Approve Chaincode for LabOrg
# ============================================================================

print_section "STEP 3: Approving chaincode for LabOrg"
...
peer lifecycle chaincode commit \
    ...
    --tlsRootCertFiles "${CRYPTO_DIR}/peerOrganizations/laborg.hot.coc.com/peers/peer0.laborg.hot.coc.com/tls/ca.crt"
...
echo "  # Set user roles (as admin)"
echo "  peer chaincode invoke -C ${CHANNEL_NAME} -n ${CHAINCODE_NAME} \\"
echo "    -c '{\"function\":\"SetUserRoles\",\"Args\":[\"LabOrgMSP|lab-gw|user:investigator1\",\"BlockchainInvestigator\"]}'"
echo ""
echo "  # Query user roles"
echo "  peer chaincode query -C ${CHANNEL_NAME} -n ${CHAINCODE_NAME} \\"
echo "    -c '{\"function\":\"GetUserRoles\",\"Args\":[\"LabOrgMSP|lab-gw|user:investigator1\"]}'"
echo ""
